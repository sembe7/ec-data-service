...
const PushNotificationEdit = inject('pushNotificationStore')
(observer(({pushNotificationStore, history, match: {params}}) => {
  const [form] = Form.useForm()

  useEffect(() => {
    pushNotificationStore.fetch({id: params.id})
  }, [])

  const handleCreate = values => {
    const payload = Object.assign(values, {id: params.id})

    pushNotificationStore.update(payload)
      .then(response => {
        if (response.result === true) {
          message.success('Push notification амжилттай засварлалаа')
        } else {
          message.error(`Push notification засварлахад алдаа гарлаа: ${response.message}`)
        }
        history.push('/mobile/push-notification')
      })
      .catch(e => {
        message.error(`Push notification засварлахад алдаа гарлаа: ${e.message}`)
      })
  }
  ...
  return <Card title='Push notification бүртгэх' bordered={false}>
    <Form
      ...
      initialValues={pushNotificationStore.current}
    >
      <FormItem
        label='Төрөл'
        name='type'
      ...
      </FormItem>
      {
        ...
        <FormItem
          label='Илгээх огноо'
          name='scheduledDate'
          initialValue={pushNotificationStore.current && moment(pushNotificationStore.current.scheduledDate)}
          {...formItemLayout}
        >
      ...
      <FormItem
        ...
        {...formItemLayout}
      >
        <Input placeholder='Username | Device token'/>
      </FormItem>
      <FormItem
        label='Priority'
        name='priority'
